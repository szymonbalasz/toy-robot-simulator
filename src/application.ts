...
export function checkRobotStateAndExecute({command, args}: {
    command: string;
    args: string[]
}, robot: Robot, rl: readline.Interface | null) {
    if (!robot.isPlaced() && command !== COMMANDS.PLACE) {
        console.log('Robot is not placed yet. Please use the PLACE command to place the robot on the grid.');
        if (rl) rl.prompt();
        return;
    }
